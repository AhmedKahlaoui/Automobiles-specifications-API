...
export function BrowseSeriePage() {
  ...
  const q = useQuery({
    queryKey: ['browse-serie-cars', serie, page, perPage],
    queryFn: () => browseService.carsBySerie(serie, { page, per_page: perPage }),
    enabled: Boolean(serie)
  })
  ...
  return (
    <Page
      title={serie ? `Serie: ${serie}` : 'Serie'}
      subtitle={q.data ? `${q.data.total} cars • page ${q.data.page}/${q.data.pages}` : 'Browse cars in this serie'}
    >
      <div style={{ display: 'grid', gap: 12 }}>
        ...
        {q.data && q.data.total === 0 ? <Alert tone="info">No cars found for this serie.</Alert> : null}
